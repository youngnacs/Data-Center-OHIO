"""Command-line planner for Heartland Super Campus."""
...
from heartland.tariff import aep_contract_term_years, aep_minimum_billing_kw
...
def cmd_plan(year: int) -> int:
    ...
    print(f"  Buildings     {snap.building_counts}")
    ...
    print(f"  Grid / BTM    {_fmt_mw(snap.grid_mw)} / {_fmt_mw(snap.btm_mw)}")
    print(f"  Interconnect  {_fmt_mw(snap.contracted_interconnection_mw)} contracted")
    print(f"  Capex (shell) {_fmt_usd(snap.total_capex_usd)}  [buildings {_fmt_usd(snap.building_capex_usd)}]")
    return 0


def cmd_roadmap() -> int:
    plan = CampusPlan()
    print(f"{'Phase':<6} {'Years':<13} {'IT MW':>10} {'Site MW':>10} {'Racks':>10} {'Capex':>12}")
    for phase, snap in zip(plan.phases, plan.roadmap(), strict=True):
        ...
        print(
            ...
            f"{snap.racks:10,} {_fmt_usd(snap.total_capex_usd):>12}"
        )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="heartland",
        description="Heartland Super Campus 20-year planning model",
    )
    sub = parser.add_subparsers(dest="cmd", required=True)

    plan = sub.add_parser("plan", help="capacity snapshot for a given year")
    plan.add_argument("--year", type=int, default=2046)
    ...
    dump.add_argument("--year", type=int, default=2046)
